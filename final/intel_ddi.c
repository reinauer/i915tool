...
void intel_prepare_ddi_buffers(struct drm_device *dev, enum port port, bool use_fdi_mode)
{
	...
	fprintf(stderr, "Initializing DDI buffers for port %c in %s mode\n",
			port_name(port),
			use_fdi_mode ? "FDI" : "DP");
...
static  long hsw_ddi_buf_ctl_values[] = {
...
void hsw_fdi_link_train(struct drm_crtc *crtc)
{
	...
	for (i=0; i < ARRAY_SIZE(hsw_ddi_buf_ctl_values); i++) {
		...
		if (temp & DP_TP_STATUS_AUTOTRAIN_DONE) {
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "BUF_CTL training done on %d step\n", i);
			...
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "Error training BUF_CTL %d\n", i);
...
void intel_ddi_init(struct drm_device *dev, enum port port)
{
	...
	case PORT_A:
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Found digital output on DDI port A\n");
		break;
	...
	default:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "No handlers defined for port %d, skipping DDI initialization\n",
				port);
		break;
	}
}
...
static  struct wrpll_tmds_clock wrpll_tmds_clock_table[] = {
	{19750,	38,	25,	18},
	{20000,	48,	32,	18},
	{21000,	36,	21,	15},
	{21912,	42,	29,	17},
				...
				struct drm_display_mode *adjusted_mode)
{
	 ...
	 */
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "Preparing HDMI DDI mode for Haswell on port %c, pipe %c\n", port_name(port), pipe_name(pipe));
	...
	if (wrpll_tmds_clock_table[i].clock != crtc->mode.clock) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "WR PLL: using settings for %dKHz on %dKHz mode\n",
			 wrpll_tmds_clock_table[i].clock, crtc->mode.clock);
	}

	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "WR PLL: %dKHz refresh rate with p=%d, n2=%d r2=%d\n",
	...
	if (intel_hdmi->has_audio) {
		 ...
		 */
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "HDMI audio on pipe %c not yet supported on DDI\n",
	...
	default:
		if (1) { 
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "%d bpp unsupported by pipe DDI function\n",
		     intel_crtc->bpp);
		}
	}
			    ...
			    enum pipe *pipe)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "No pipe for ddi port %i found\n", intel_hdmi->ddi_port);
