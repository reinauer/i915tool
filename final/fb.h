...
struct fb_deferred_io {
	...
	unsigned long delay;/* mutex that protects the page list */
...
struct fb_ops {
	...
	ssize_t (*fb_write)(struct fb_info *info,  char  *buf,
			    size_t count, loff_t *ppos);
...
struct fb_info {
	int node;
	int flags;		/* Lock for open/release/ioctl funcs */
	/* Lock for fb_mmap and smem_* fields */
	struct fb_var_screeninfo var;	/* Current var */
	struct fb_fix_screeninfo fix;	/* Current fix */
	struct fb_monspecs monspecs;	/* Current Monitor specs *//* Framebuffer event queue */
...
static inline struct apertures_struct *alloc_apertures(unsigned int max_num) {
	struct apertures_struct *a = allocz(sizeof(struct apertures_struct) + max_num * sizeof(struct aperture));
	if (!a)
		return NULL;
	a->count = max_num;
	return a;
 ...
 */
extern void sys_fillrect(struct fb_info *info,  struct fb_fillrect *rect);
extern void sys_copyarea(struct fb_info *info,  struct fb_copyarea *area);
extern void sys_imageblit(struct fb_info *info,  struct fb_image *image);
extern ssize_t fb_sys_read(struct fb_info *info, char  *buf,
			   size_t count, loff_t *ppos);
extern ssize_t fb_sys_write(struct fb_info *info,  char  *buf,
			    size_t count, loff_t *ppos);
...
extern int fbmon_dpms( struct fb_info *fb_info);
extern  unsigned char *fb_firmware_edid(struct device *device);
...
extern int fb_add_videomode( struct fb_videomode *mode,
...
extern  char *fb_mode_option;
extern  struct fb_videomode vesa_modes[];
extern  struct fb_videomode cea_modes[64];
