...
static  intel_limit_t intel_limits_i8xx_dvo = {
...
static  intel_limit_t intel_limits_i8xx_lvds = {
	.dot = { .min = 25000, .max = 350000 },
	.vco = { .min = 930000, .max = 1400000 },
	.n = { .min = 3, .max = 16 },
	.m = { .min = 96, .max = 140 },
...
static  intel_limit_t intel_limits_i9xx_sdvo = {
...
static  intel_limit_t intel_limits_i9xx_lvds = {
	.dot = { .min = 20000, .max = 400000 },
	.vco = { .min = 1400000, .max = 2800000 },
	.n = { .min = 1, .max = 6 },
	.m = { .min = 70, .max = 120 },
...
static  intel_limit_t intel_limits_pineview_sdvo = {
	.dot = { .min = 20000, .max = 400000},
...
static  intel_limit_t intel_limits_pineview_lvds = {
	.dot = { .min = 20000, .max = 400000 },
	.vco = { .min = 1700000, .max = 3500000 },
	.n = { .min = 3, .max = 6 },
	.m = { .min = 2, .max = 256 },
 ...
 */
static  intel_limit_t intel_limits_ironlake_dac = {
	.dot = { .min = 25000, .max = 350000 },
...
static  intel_limit_t intel_limits_ironlake_single_lvds = {
	.dot = { .min = 25000, .max = 350000 },
	.vco = { .min = 1760000, .max = 3510000 },
	.n = { .min = 1, .max = 3 },
	.m = { .min = 79, .max = 118 },
...
static  intel_limit_t intel_limits_ironlake_dual_lvds = {
	.dot = { .min = 25000, .max = 350000 },
	.vco = { .min = 1760000, .max = 3510000 },
	.n = { .min = 1, .max = 3 },
	.m = { .min = 79, .max = 127 },
...
static  intel_limit_t intel_limits_ironlake_dual_lvds_100m = {
	.dot = { .min = 25000, .max = 350000 },
	.vco = { .min = 1760000, .max = 3510000 },
	.n = { .min = 1, .max = 3 },
	.m = { .min = 79, .max = 126 },
...
static  intel_limit_t *intel_ironlake_limit(struct drm_crtc *crtc,
						int refclk)
{
...
static  intel_limit_t *intel_limit(struct drm_crtc *crtc, int refclk)
{
	struct drm_device *dev = crtc->dev;
	 intel_limit_t *limit;
				     ...
				     enum pipe pipe)
{
	...
	if (wait_for((I915_READ(reg) & TRANS_STATE_ENABLE) == 0, 50)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "failed to disable transcoder %d\n", pipe);
	}
}
				    ...
				    enum pipe pipe)
{
	...
	if (lvds_pipe_enabled(dev_priv, val, pipe)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "disable lvds on pipe %d val 0x%08x\n", pipe, val);
...
static void ironlake_enable_fbc(struct drm_crtc *crtc, unsigned long interval)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "enabled fbc on plane %d\n", intel_crtc->plane);
}
...
static void ironlake_crtc_dpms(struct drm_crtc *crtc, int mode)
{
	...
	case DRM_MODE_DPMS_SUSPEND:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "crtc %d/%d dpms on\n", pipe, plane);
		ironlake_crtc_enable(crtc);
		break;

	case DRM_MODE_DPMS_OFF:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "crtc %d/%d dpms off\n", pipe, plane);
...
static void i9xx_crtc_dpms(struct drm_crtc *crtc, int mode)
{
	...
	case DRM_MODE_DPMS_SUSPEND:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr,
			"Warning: somebody tried to call i9xx_crtc_enable\n");
		break;
	case DRM_MODE_DPMS_OFF:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr,
...
static void intel_crtc_dpms(struct drm_crtc *crtc, int mode)
{
	...
	default:
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Can't update pipe %c in SAREA\n", pipe_name(pipe));
		break;
	}
}
...
{
	static  int sr_latency_ns = 12000;
	struct drm_i915_private *dev_priv = dev->dev_private;
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "Setting FIFO watermarks - A: plane=%d, cursor=%d, B: plane=%d, cursor=%d, SR: plane=%d, cursor=%d\n",
		      planea_wm, cursora_wm,
		      planeb_wm, cursorb_wm,
		      plane_sr, cursor_sr);
				     ...
				     struct drm_crtc *crtc)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "ELD on pipe %c\n", pipe_name(i));

	i = I915_READ(aud_cntl_st);
	i = (i >> 29) & 0x3;		/* DIP_Port_Select, 0x1 = PortB */
	if (!i) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Audio directed to unknown port\n");
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "ELD on port %c\n", 'A' + i);
		eldv = IBX_ELD_VALIDB << ((i - 1) * 4);
	}

	if (intel_pipe_has_type(crtc, INTEL_OUTPUT_DISPLAYPORT)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "ELD: DisplayPort detected\n");
		     ...
		     struct drm_display_mode *mode)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "ELD on [CONNECTOR:%d:%s], [ENCODER:%d:%s]\n",
			 connector->base.id,
			 ...
			 struct drm_i915_gem_object *obj)
{
	...
	intel_fb = allocz(sizeof(*intel_fb));
	if (!intel_fb) {
				...
				struct intel_load_detect_pipe *old)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "[CONNECTOR:%d:%s], [ENCODER:%d:%s]\n",
	...
	if (!crtc) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "no pipe available for load-detect\n");
		return false;
	}
	...
	if (crtc->fb == NULL) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "creating tmp fb for load-detection\n");
		crtc->fb = intel_framebuffer_create_for_mode(dev, mode, 24, 32);
		old->release_fb = crtc->fb;
	} else
		{
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "reusing fbdev for load-detection framebuffer\n");
		}
	if (IS_ERR(crtc->fb)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "failed to allocate framebuffer for load-detection\n");
		crtc->fb = old_fb;
		return false;
	}
	...
	if (!drm_crtc_helper_set_mode(crtc, mode, 0, 0, old_fb)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "failed to set mode on load-detect pipe\n");
...
static int intel_crtc_clock_get(struct drm_device *dev, struct drm_crtc *crtc)
{
	...
	if (!IS_GEN2(dev)) {
		...
		default:
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "Unknown DPLL mode %08x in programmed "
...
static void intel_crtc_init(struct drm_device *dev, int pipe)
{
	...
	intel_crtc = allocz(sizeof(struct intel_crtc) + (INTELFB_CONN_LIMIT * sizeof(struct drm_connector *)));
	if (intel_crtc == NULL)
		return;
	...
	if (IS_MOBILE(dev) && IS_GEN3(dev)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "swapping pipes & planes for FBC\n");
		intel_crtc->plane = !pipe;
	}
...
bool has_edp_a(struct drm_device *dev)
{
	struct drm_i915_private *dev_priv = dev->dev_private;
			   ...
			   struct drm_i915_gem_object *obj)
{
	...
	if (ret) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "framebuffer init failed %d\n", ret);
		return ret;
	}
...
void ironlake_enable_drps(struct drm_device *dev)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "fmax: %d, fmin: %d, fstart: %d\n",
			 fmax, fmin, fstart);
	...
	if (wait_for((I915_READ(MEMSWCTL) & MEMCTL_CMD_STS) == 0, 10)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "stuck trying to change perf mode\n");
	}
	mdelay(1);
...
static int intel_enable_rc6(struct drm_device *dev)
{
	...
	if (INTEL_INFO(dev)->gen == 6) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Sandybridge: deep RC6 disabled\n");
		return INTEL_RC6_ENABLE;
	}
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "RC6 and deep RC6 enabled\n");
...
void intel_modeset_init(struct drm_device *dev)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "%d display pipe%s available.\n",
	...
	for (i = 0; i < dev_priv->num_pipe; i++) {
		...
		if (ret) {
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "plane %d init failed: %d\n", i, ret);
		}
	}
