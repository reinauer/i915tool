		  ...
		  struct drm_display_mode *adjusted_mode)
{
	...
	if (intel_dp->has_audio) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Enabling DP audio on pipe %c\n",
...
intel_dp_detect(struct drm_connector *connector, bool force)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "DPCD: %02hx%02hx%02hx%02hx%02hx%02hx%02hx%02hx\n",
...
static  struct drm_encoder_helper_funcs intel_dp_helper_funcs = {
	.dpms = intel_dp_dpms,
...
static  struct drm_connector_funcs intel_dp_connector_funcs = {
...
static  struct drm_connector_helper_funcs intel_dp_connector_helper_funcs = {
	.get_modes = intel_dp_get_modes,
	.mode_valid = intel_dp_mode_valid,
	.best_encoder = intel_best_encoder,
};

static  struct drm_encoder_funcs intel_dp_enc_funcs = {
};
...
intel_dp_init(struct drm_device *dev, int output_reg)
{
	...
	intel_dp = allocz(sizeof(struct intel_dp));
	if (!intel_dp)
		return;
	...
	if (is_edp(intel_dp)) {
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "cur t1_t3 %d t8 %d t9 %d t10 %d t11_t12 %d\n",
			      cur.t1_t3, cur.t8, cur.t9, cur.t10, cur.t11_t12);

		vbt = dev_priv->edp.pps;

		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "vbt t1_t3 %d t8 %d t9 %d t10 %d t11_t12 %d\n",
...
#define get_delay(field)	((max(cur.field, vbt.field) + 9) / 10)
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "panel power up delay %d, power down delay %d, power cycle delay %d\n",
			      intel_dp->panel_power_up_delay, intel_dp->panel_power_down_delay,
			      intel_dp->panel_power_cycle_delay);

		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "backlight on delay %d, off delay %d\n",
		...
		if (ret) {
			...
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "failed to retrieve link info, disabling eDP\n");
			return;
		}
	}
