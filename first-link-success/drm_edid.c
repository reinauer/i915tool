		      ...
		      int block, int len)
{
	...
	do {
		...
		if (ret == -ENXIO) {
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "drm: skipping non-existent adapter %s\n",
					adapter->name);
			break;
		}
	} while (ret != 2 && --retries);
...
carp:
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "%s: EDID block %d invalid.\n",
		 drm_get_connector_name(connector), j);
			       ...
			       struct edid *edid)
{
	...
	list_for_each_entry_safe(cur_mode, t, &connector->probed_modes, head) {
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Adding LVDS downclock mode\n");
		return 1;
	}
	return 0;
}
						  ...
						  u32 quirks)
{
	...
	if (pt->misc & DRM_EDID_PT_STEREO) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, KERN_WARNING "stereo mode not supported\n");
		return NULL;
	}
	if (!(pt->misc & DRM_EDID_PT_SEPARATE_SYNC)) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, KERN_WARNING "composite sync not supported\n");
	}

	/* it is incorrect if hsync/vsync width is zero */
	if (!hsync_pulse_width || !vsync_pulse_width) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Incorrect Detailed timing. "
				"Wrong Hsync/Vsync pulse width\n");
		return NULL;
	}
	mode = malloc(sizeof(struct drm_display_mode));
	if (!mode)
		return NULL;
...
parse_hdmi_vsdb(struct drm_connector *connector, uint8_t *db)
{
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "HDMI: DVI dual %d, "
		    "max TMDS clock %d, "
		    "latency present %d %d, "
		    "video latency %d %d, "
		    "audio latency %d %d\n",
...
void drm_edid_to_eld(struct drm_connector *connector, struct edid *edid)
{
	...
	if (!cea) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "ELD: no CEA Extension found\n");
		return;
	}
	...
	fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
	fprintf(stderr, "ELD size %d, SAD count %d\n", (int)eld[2], sad_count);
}
...
bool drm_detect_monitor_audio(struct edid *edid)
{
	...
	if (has_audio) {
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "Monitor has basic audio support\n");
		goto end;
	}
		...
		if ((edid_ext[i] >> 5) == AUDIO_BLOCK) {
			...
			for (j = 1; j < (edid_ext[i] & 0x1f); j += 3) {
				fprintf(stderr, "[000000.0] [drm:%s], ",
					__func__);
				fprintf(stderr, "CEA audio format %d\n",
					      (edid_ext[i + j] >> 3) & 0xf);
			}
			goto end;
		}
