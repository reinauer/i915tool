		       ...
		       struct bdb_header *bdb)
{
	...
	if (general) {
		...
		fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
		fprintf(stderr, "BDB_GENERAL_FEATURES int_tv_support %d int_crt_support %d lvds_use_ssc %d lvds_ssc_freq %d display_clock_mode %d\n",
			  ...
			  struct bdb_header *bdb)
{
	...
	for (i = 0; i < child_device_num; i++) {
		...
		if (!p_mapping->initialized) {
			...
			fprintf(stderr, "[000000.0] [drm:%s], ", __func__);
			fprintf(stderr, "SDVO device: dvo=%x, addr=%x, wiring=%d, ddc_pin=%d, i2c_pin=%d\n",
				      p_mapping->dvo_port,
